// ICF Coaching Reading Materials Database
// 40 High-Quality Articles for Management Coaching
...
export const readingArticles: Record<string, ReadingArticle> = {
  // Leadership & Management (10 articles)
  '1': {
    id: '1',
  ...
  '5': {
    ...
    content: [
      ...
      '**Delegeringsmatrisen:**',
      'Urgent + Viktig = Gör själv eller delegera till din mest erfarna',
      'Inte urgent + Viktig = Perfect att delegera för utveckling',
      'Urgent + Inte viktig = Delegera eller eliminera',
      'Inte urgent + Inte viktig = Sluta göra',
      '**Reflektionsfrågor:**',
      '- Vilka uppgifter gör du som någon annan kunde göra?',
      '- Vem i ditt team behöver utmanas mer?',
      '- Vad hindrar dig från att delegera mer?',
      '**Övning denna vecka:**',
      'Identifiera tre uppgifter att delegera. Välj en person för varje. Boka tid för en tydlig delegeringskonversation med klara förväntningar och uppföljningspunkter.'
    ]
  },
  ...
  '6': {
    ...
    content: [
      ...
      '**Övningar för bättre lyssning:**',
      '**Övning 1: Två minuters lyssning**',
      'Be en kollega prata i två minuter om något viktigt för dem. Din uppgift: Lyssna bara. Ingen respons förrän tiden är ute.',
      '**Övning 2: Reflekterande lyssning**',
      'I nästa samtal, försök parafrasera varje viktig punkt personen gör innan du svarar.',
      '**Övning 3: Känsloreflektioner**',
      'Öva att namnge den känsla du uppfattar: "Det låter som att du är..." Kolla om du har rätt.',
      '**Barriärer för aktiv lyssning:**',
      '- **Stress** - Du är distraherad av egna bekymmer',
      '- **Domdera** - Du har redan bestämt vad du tycker',
      '- **Fixa-läge** - Du vill lösa problemet snabbt',
      '- **Egocentrism** - Du relaterar allt till dig själv',
      '**Minneslista för aktiv lyssning:**',
      '- Pausa din inre monolog',
      '- Ställ ingen motfråga',
      '- Sök inte bekräftelse på din världsbild',
      ...
      '- Konflikter minskar',
      '**Reflektionsfrågor:**',
      '- Hur ofta lyssnar du på nivå 3?',
      '- Vem i ditt liv behöver bli lyssnad på mer?',
      '- Vad händer med dig när någon verkligen lyssnar på dig?'
    ]
  },

  // Continue with remaining 34 articles...
  // I'll add more in next messages due to token limits
};

// Helper function to get articles by category
export function getArticlesByCategory(category?: string): ReadingArticle[] {
  const articles = Object.values(readingArticles);
...
export function getArticleById(id: string): ReadingArticle | undefined {
  return readingArticles[id];
}

// Helper function to get categories
export function getCategories(): string[] {
  const categories = new Set(Object.values(readingArticles).map(a => a.category));
